...
PokemonCenter::~PokemonCenter(){
  ...
  unsigned int PokemonCenter::DistributeStamina(unsigned int points_needed =0){
      if(num_stamina_points_remaining >= points_needed){
        num_stamina_points_remaining -= points_needed;
        return points_needed;
      }
      else{
          int i = num_stamina_points_remaining;
  ...
  void PokemonCenter::ShowStatus(){
      ...
      Building :: ShowStatus();
        ...
        << endl << "\tHas " << num_stamina_points_remaining << " stamina point(s) remaining" << endl;
  }